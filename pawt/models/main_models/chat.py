...
class Chat(PAWTLazy):
    ...
    def export_invite_link(self):
        return self._tg.get(
            API_PATH["export_chat_invite_link"], params=dict(chat_id=self.id)
        )
    ...
    def get_member(self, user):
        ...
        data = self._tg.get(
            API_PATH["get_chat_member"], params=dict(chat_id=self.id, user_id=user_id)
        )
        return ChatMember(self._tg, data)

    def get_administrators(self):
        data = self._tg.get(
            API_PATH["get_chat_administrators"], params=dict(chat_id=self.id)
        )
    ...
    def delete_photo(self):
        return self._tg.post(API_PATH["delete_chat_photo"], data=dict(chat_id=self.id))

    def set_title(self, title):
        return self._tg.post(
            API_PATH["set_chat_title"], data=dict(chat_id=self.id, title=title)
        )
    ...
    def set_sticker_set(self, sticker_set):
        ...
        return self._tg.post(
            API_PATH["set_chat_sticker_set"],
            data=dict(chat_id=self.id, sticker_set_name=sticker_set),
        )

    def delete_sticker_set(self):
        return self._tg.post(
            API_PATH["delete_chat_sticker_set"], data=dict(chat_id=self.id)
        )

    def set_description(self, description):
        return self._tg.post(
            API_PATH["set_chat_description"],
            data=dict(chat_id=self.id, description=description),
        )

    def set_photo(self, photo):
        return self._tg.post(
            API_PATH["set_chat_photo"],
    ...
    def unban_member(self, user):
        user_id = self._coerce_userlike_obj(user)
        return self._tg.post(
            API_PATH["unban_chat_member"], data=dict(chat_id=self.id, user_id=user_id)
        )
    ...
    def get_member_count(self):
        return self._tg.get(
            API_PATH["get_chat_members_count"], params=dict(chat_id=self.id)
        )

    def unpin_message(self):
        return self._tg.post(API_PATH["unpin_chat_message"], data=dict(chat_id=self.id))

    def _file_post_helper(
        self, api_path, data, possible_file, file_param_name, files=None
    ):
        ...
        if caption and len(caption) > MAX_LENGTH["caption"]:
            msg = "Caption is too long ({} > {})".format(
                len(caption), MAX_LENGTH["caption"]
            )
    ...
    def _post_and_return_message(self, api_path, data, files=None):
        ...
        if text and len(text) > MAX_LENGTH["text"]:
            msg = "Text is too long ({} > {})".format(len(text), MAX_LENGTH["text"])
    ...
    def send_document(
        ...
        return self._file_post_helper(
            API_PATH["send_document"], info, document, "document"
        )
    ...
    def send_video(
        ...
        if caption:
            info["caption"] = caption
        return self._file_post_helper(API_PATH["send_video"], info, video, "video")
    ...
    def send_voice(
        ...
        if duration:
            info["duration"] = duration
        return self._file_post_helper(API_PATH["send_voice"], info, voice, "voice")
    ...
    def send_photo(
        ...
        if caption:
            info["caption"] = caption
        return self._file_post_helper(API_PATH["send_photo"], info, photo, "photo")
    ...
    def send_contact(
        ...
        if contact and (phone_number or first_name or last_name):
            raise BadArgument(
                "If contact is provided, phone_number, "
    ...
    def send_media_group(self, media, disable_notification=None, reply_to=None):
        ...
        for thing in media:
            if isinstance(thing, dict):
                ...
                builder.build_input_media_video(
                    thing,
                    width=thing.width,
                    height=thing.height,
                ...
                raise BadArgument(
                    "Cannot handle thing of type {} as "
        ...
        response = self._tg.post(
            API_PATH["send_media_group"], data=info, files=builder.files
        )
    ...
    def send_venue(
        ...
        if venue and any((latitude, longitude, title, address, foursquare_id)):
            raise BadArgument(
                "If venue is provided, latitude, longitude, "
    ...
    def send_audio(
        ...
        if title:
            info["title"] = title
        return self._file_post_helper(API_PATH["send_audio"], info, audio, "audio")
    ...
    def send_location(
        ...
        if location and (latitude or longitude):
            raise BadArgument(
                "If location is provided, latitude and "
    ...
    def send_video_note(
        ...
        if length:
            info["length"] = length
        return self._file_post_helper(
            API_PATH["send_video_note"], info, video_note, "video_note"
        )

    def send_chat_action(self, action):
        if action not in Chat.CHAT_ACTIONS:
            raise BadArgument("Action must be one of {!r}".format(Chat.CHAT_ACTIONS))
    ...
    def send_sticker(
        self, sticker, disable_notification=None, reply_to=None, reply_markup=None
    ):
        info = self._send_helper(disable_notification, reply_to, reply_markup)
        if hasattr(sticker, "file"):
            sticker = sticker.file.id
        return self._file_post_helper(
            API_PATH["send_sticker"], info, sticker, "sticker"
        )
...
class MediaGroupBuilder:
    ...
    def build_input_media(
        self, type_, media, caption=None, width=None, height=None, duration=None
    ):
    ...
    def build_input_media_video(
        self, media, caption=None, width=None, height=None, duration=None
    ):
