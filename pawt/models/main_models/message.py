...
class Message(PAWTBase):
    ...
    def edit(
        ...
        if (self.text or self.game) and new_text:
            ...
            if parse_mode or disable_web_page_preview:
                raise BadType(
                    "This Message doesn't support the parameters "
                    "parse_mode or disable_web_page_preview."
                )
            if new_text:
                # we're editing something with a caption
                data["caption"] = new_text
                response = self._tg.post(API_PATH["edit_message_caption"], data=data)
            else:
                # we're only editing the reply_markup
                if not reply_markup:
                    raise BadArgument(
                        "At least new_text or reply_markup must " "be provided."
                    )
                response = self._tg.post(
                    API_PATH["edit_message_reply_markup"], data=data
                )
    ...
    def get_game_high_scores(self, user):
        ...
        return [GameHighScore(self._tg, data=ghs) for ghs in response]

    def set_game_score(self, user, score, force=False, disable_edit_message=False):
...
class MessageReplier(PAWTBase):
    ...
    *args and **kwargs…"""
    ...
    def send_sticker(self, sticker, disable_notification=None, reply_markup=None):
        return self.chat.send_sticker(
            sticker, disable_notification, self.message_id, reply_markup
        )
    ...
    def send_location(
        ...
        return self.chat.send_location(
            location,
            latitude,
            longitude,
            live_period,
    ...
    def send_photo(
        self, photo, caption=None, disable_notification=None, reply_markup=None
    ):
        return self.chat.send_photo(
            photo, caption, disable_notification, self.message_id, reply_markup
        )

    def send_media_group(
        self, media, disable_notification=None,
    ):
        return self.chat.send_media_group(media, disable_notification, self.message_id)

    def send_game(self, game_short_name, disable_notification=None, reply_markup=None):
        return self.chat.send_game(
            game_short_name, disable_notification, self.message_id, reply_markup
        )
    ...
    def send_document(
        self, document, caption=None, disable_notification=None, reply_markup=None
    ):
        return self.chat.send_document(
            document, caption, disable_notification, self.message_id, reply_markup
        )
    ...
    def send_video_note(
        ...
        return self.chat.send_video_note(
            video_note,
            duration,
            length,
            disable_notification,
