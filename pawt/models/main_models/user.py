...
class User(PAWTBase):
    ...
    def __eq__(self, other):
        return (hasattr(other, "id") and str(self.id) == str(other.id)) or str(
            self.id
        ) == str(other)
    ...
    def create_new_sticker_set(
        self, name, title, png_sticker, emojis, mask_position=None
    ):

        data = dict(user_id=self.id, name=name, title=title, emojis=emojis)
    ...
    def get_profile_photos(self, offset=None, limit=None):
        ...
        if limit:
            data["limit"] = limit
        response = self._tg.get(API_PATH["get_user_profile_photos"], params=data)
        return UserProfilePhotos(self._tg, response)

    def upload_sticker_file(self, png_sticker):
        data = self._tg.post(
            API_PATH["upload_sticker_file"],
