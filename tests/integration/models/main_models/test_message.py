...
def test_edit_live_location():
    with bm.use_cassette("test_message__test_edit_live_location"):
        message = chat.send_location(
            latitude=37.8720789, longitude=-122.258008, live_period=120
        )
        message.edit_live_location(47.6205063, -122.3514661, reply_markup=REPLY_MARKUP)
...
def test_replies():
    with bm.use_cassette("test_message__test_replies"):
        ...
        assert repr(parent.reply) == "<MessageReplier for {}>".format(repr(parent))
        ...
        parent.reply.send_venue(
            latitude=37.8720789,
            longitude=-122.258008,
            title="The Campanile",
            address="Sather Tower, Berkeley, CA 94720",
        )
        parent.reply.send_contact(phone_number="+15555555555", first_name="John")
        try:
            parent.reply.send_invoice(
                "Buy stuff",
                "description",
                "abc123",
                "saldfkjasd;lfkja",
        ...
        parent.reply.send_document("http://faculty.weber.edu/cbergeson/3060/denevi.pdf")
...
def test_sticker_replies():
    ...
    with bm.use_cassette("test_message__test_sticker_replies"):
        ...
        u.create_new_sticker_set(
            name, "test_message.py", "https://i.imgur.com/BDZfRJg.png", "🍊"
        )
