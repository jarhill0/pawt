...
pic = 'https://upload.wikimedia.org/wikipedia/commons/thumb/1/10/Tursiops_' \
      'truncatus_01.jpg/800px-Tursiops_truncatus_01.jpg'


def check_text(messages, text):
    return (text.strip() == ' '.join(m.text for m in messages).strip() or
    ...
    with bm.use_cassette('test_exceptions__test_toolong_with_caption'):
        for caption in captions:
            try:
                ...
                assert (caption.strip() == sent_opt_1.strip() or
                        caption.strip() == sent_opt_2.strip())
