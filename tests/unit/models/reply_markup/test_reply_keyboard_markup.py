...
def test_false_args():
    builder = ReplyKeyboardMarkupBuilder()
    assert {"keyboard": [[]], "selective": False} == builder.build(selective=False)

    builder.add_button("Only location", request_contact=False, request_location=True)
...
def test_num_args():
    builder = ReplyKeyboardMarkupBuilder()
    try:
        builder.add_button("should fail", request_contact=True, request_location=True)
